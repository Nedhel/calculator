...
function setDotOnScreen(id){//this is to avoid repet a dot
...
function clearAll(){//both screens
...
function operation(text){
    ...
    if(getScreen('screenOperation')!='' && getScreen('screen')!=''){
        equal()        
    } else if(getScreen('screenOperation')!='' && getScreen('screen')==''){        
        let sOperation = getScreen('screenOperation').split('')
        let sign = sOperation[sOperation.length - 1]
        switch(sign){
            case '+': setCharOnScreen('screen',0)
                break;
            case '-': setCharOnScreen('screen',0)
                break;
            case '/': setCharOnScreen('screen',1)
                break;
            case '*': setCharOnScreen('screen',1)
                break;
        }
